...
type ProductDoc = Omit<Product, "id" | "createdAt" | "updatedAt">;
...
function getProductImage(product: ProductSeed): string {
  const imageQueries: Record<string, string> = {
    ...
    "Luz Nocturna Nube": "baby,nursery,nightlight",
    "Móvil de Cuna de Madera": "baby,crib,mobile",
    "Set de Arrullo y Manta": "baby,swaddle,blanket",

    // JUGUETES
    "Mordedor de Madera Natural": "baby,wooden,teether",
    "Mordedor de Silicona Arena": "baby,silicone,teether",
    "Sonajero de Madera": "baby,wooden,rattle",
    "Sonajero de Tela": "baby,fabric,rattle",
  ...
  const query = imageQueries[product.name] ?? "baby,products";

  return `https://loremflickr.com/600/600/${query}?lock=${slugify(
    product.name
  )}`;
}
...
async function seed() {
  ...
  for (const product of CATALOG) {
    ...
    const ref = doc(productsCollection, slugify(product.name));
